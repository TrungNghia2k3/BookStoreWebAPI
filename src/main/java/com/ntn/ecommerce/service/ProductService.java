...
public class ProductService {
    ...
    public ProductResponse createProduct(
        ...
        if (productRepository.existsByTitleOrIsbn10OrIsbn13(
                request.getTitle(), request.getIsbn10(), request.getIsbn13())) {
        ...
        if (imageMain != null && !imageMain.isEmpty()) {
            String mainImagePath =
                    imageUtilities.saveFile("product", product.getId(), imageMain.getOriginalFilename(), imageMain);
        ...
        if (imageSubOne != null && !imageSubOne.isEmpty()) {
            String subImagePath =
                    imageUtilities.saveFile("product", product.getId(), imageSubOne.getOriginalFilename(), imageSubOne);
        ...
        if (imageSubTwo != null && !imageSubTwo.isEmpty()) {
            String subImagePath =
                    imageUtilities.saveFile("product", product.getId(), imageSubTwo.getOriginalFilename(), imageSubTwo);
            product.setImageSubTwo(subImagePath);
        }

        // Handle audio upload if provided
        if (audio != null && !audio.isEmpty()) {
            String audioPath = audioUtilities.saveAudioFile(product.getId(), audio.getOriginalFilename(), audio);
    ...
    public ProductResponse updateProduct(
        ...
        if (imageMain != null
                && !imageMain.isEmpty()
                && product.getImageMain() != null
                && !product.getImageMain().isEmpty()) {
            imageUtilities.deleteFile("product", id, product.getImageMain());
        }

        if (imageSubOne != null
                && !imageSubOne.isEmpty()
                && product.getImageSubOne() != null
                && !product.getImageSubOne().isEmpty()) {
            imageUtilities.deleteFile("product", id, product.getImageSubOne());
        }
        if (imageSubTwo != null
                && !imageSubTwo.isEmpty()
                && product.getImageSubTwo() != null
                && !product.getImageSubTwo().isEmpty()) {
            imageUtilities.deleteFile("product", id, product.getImageSubTwo());
        }

        // Xóa tệp âm thanh cũ nếu có tệp mới và tệp cũ tồn tại
        if (audio != null
                && !audio.isEmpty()
                && product.getAudio() != null
                && !product.getAudio().isEmpty()) {
            audioUtilities.deleteAudioFile(id, product.getAudio());
        }
        ...
        if (imageMain != null && !imageMain.isEmpty()) {
            String mainImagePath = imageUtilities.saveFile(
                    "product", String.valueOf(product.getId()), imageMain.getOriginalFilename(), imageMain);
            product.setImageMain(mainImagePath);
        }

        if (imageSubOne != null && !imageSubOne.isEmpty()) {
            String subImagePath = imageUtilities.saveFile(
                    "product", String.valueOf(product.getId()), imageSubOne.getOriginalFilename(), imageSubOne);
            product.setImageSubOne(subImagePath);
        }

        if (imageSubTwo != null && !imageSubTwo.isEmpty()) {
            String subImagePath = imageUtilities.saveFile(
                    "product", String.valueOf(product.getId()), imageSubTwo.getOriginalFilename(), imageSubTwo);
            product.setImageSubTwo(subImagePath);
        }
        ...
        if (audio != null && !audio.isEmpty()) {
            String audioFilePath =
                    audioUtilities.saveAudioFile(String.valueOf(product.getId()), audio.getOriginalFilename(), audio);
    ...
    //        List<Product> products = productRepository.findAll(Sort.by(Sort.Direction.fromString(sortDirection),
    // sortBy));
    //
    ...
    public PageResponse<ProductResponse> filterProducts(
        ...
        Page<Product> products = productRepository.filterProducts(
                categoryId, publisherId, formality, minPrice, maxPrice, title, pageable);
        Page<ProductResponse> productResponses = products.map(this::mapToProductResponse);
    ...
    public List<ProductRankingResponse> getRankingMostPopularProducts() {
        ...
        for (Cart cart : carts) {
            for (CartProduct cartProduct : cart.getCartProducts()) {
                ...
                productRankingMap
                        .get(product)
                        .setTotalQuantity(productRankingMap.get(product).getTotalQuantity() + quantity);
            }
        }

        // Convert the map to a list and sort by totalQuantity in descending order
        return productRankingMap.values().stream()
                .sorted(Comparator.comparing(ProductRankingResponse::getTotalQuantity)
                        .reversed())
                .collect(Collectors.toList());
    }
